...
![Image](images/square.jpg) **注意**显然，在现实世界中，在应用中硬编码 IP 地址被认为是一种不好的做法。在这种情况下，客户端将只能与服务器在同一台机器上运行，这在某种程度上违背了远程通信的目的。在您的情况下，客户端可能使用服务器的主机名，而不是 IP 地址(可能通过 DNS 配置)。IP 地址经常变化，有时甚至通过 DHCP 动态分配。
