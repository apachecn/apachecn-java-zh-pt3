...
`java.sql`包的`Connection`接口表示从应用程序到数据库的连接。它是应用程序和数据库通信的通道。表 [12-1](#Tab1) 列出了`Connection`界面中的重要方法。所有这些方法都抛出`SQLException` s，所以在表中没有提到。
...
现在，让我们编写一个简单的应用程序来获取连接(参见清单 [12-1](#FPar1) )。
