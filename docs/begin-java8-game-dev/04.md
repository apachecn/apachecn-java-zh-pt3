...
```java
// Program logic that gets processed on every 脉冲that JavaFX processes
...
您还了解了 JavaFX 的核心概念，如`JavaFX Scene Graph`和 JavaFX 脉冲events 系统，您将在阅读本书时利用它们来创建 Java 8 游戏，从下一章开始，我将解释如何在 NetBeans 中使用 JavaFX Scene Builder 可视化编辑工具。
