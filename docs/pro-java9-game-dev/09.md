...
因此，Image 类(object)用于准备要使用的数字图像资产，也就是说，从 URL 读取其数据，必要时调整其大小(使用您喜欢的任何平滑和纵横比锁定)，甚至在应用中进行其他操作时异步加载它。值得注意的是，这个图像类(或对象)不显示您的图像资产；它只是加载它，根据需要进行缩放，并将其放入系统内存，以便在您的应用中使用。
