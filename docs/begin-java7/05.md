...
![images](images/square.jpg) **注意** `LinkedHashSet`是`HashSet`的子类，它使用链表来存储其元素。因此，`LinkedHashSet`的迭代器按照元素被插入的顺序返回元素。例如，如果清单 5-4 中的[指定了`Set<String> ss = new LinkedHashSet<>();`，那么应用的输出将会是`ss: apples pears grapes bananas kiwis null`。此外，`LinkedHashSet`提供比`HashSet`更慢的性能和比`TreeSet`更快的性能。](#list_5_4)
...
![images](images/square.jpg) **注意**应用的各种用途的使用属性。例如，如果您的应用有一个图形用户界面，您可以通过一个`Properties`对象将其主窗口的屏幕位置和大小保存到一个文件中，这样应用就可以在下次运行时恢复窗口的位置和大小。
