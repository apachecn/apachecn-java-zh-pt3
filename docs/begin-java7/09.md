...
![images](images/square.jpg) **注意** `ServerSocket`声明了一个`void close()`方法，用于在终止服务器应用之前关闭服务器套接字。
...
![images](images/square.jpg) **注意**虽然你可能认为`Socket`和`ServerSocket`就是你需要的全部，`DatagramSocket`(及其`MulticastSocket`子类)有它们的用途。例如，考虑一个场景，其中一组机器需要偶尔告诉服务器它们还活着。偶尔丢失消息或者消息没有按时到达都没有关系。另一个例子是周期性广播股票价格的低优先级股票报价机。当一个包裹没有到达时，很可能下一个包裹会到达，然后你会收到最新价格的通知。在实时应用中，及时的交付比可靠或有序的交付更重要。
...
![images](images/square.jpg) **提示**在嵌入式环境中运行`SimpleApp`(或任何其他 Java DB 应用)时，可以通过设置`derby.system.home`属性来确定数据库目录将创建在哪里。例如，`java -Dderby.system.home=c:\ SimpleApp`导致在 Windows 平台上的 C:驱动器的根目录中创建`derbyDB`。
...
![images](images/square.jpg) **注**为了方便起见，本章的应用只使用嵌入式驱动程序连接语法。
